...
- 后台「发表记录」页面及文章详情中的字段，例如：
  ...
  - `reprint_num`：转载量
- 文章分析页中的阅读人数、平均阅读时长、完读率、点赞人数、在看人数、收藏人数、分享人数、阅读后关注人数、赞赏金额等
- 按日来源明细、文章内跳转统计和音频播放明细
- 读者用户画像：性别分布、年龄分布和地域分布（匿名聚合数据）

导出表头按数据来源区分：

- `文章详情_...`：发表记录中的文章详情字段，例如 `文章详情_广告信息`、`文章详情_文章合集信息`、`文章详情_是否开启留言`、`文章详情_封面图片`。
- `文章分析_...`：文章分析页字段，例如 `文章分析_阅读人数`、`文章分析_性别分布`、`文章分析_年龄分布`、`文章分析_地域分布`。

用户画像来自微信后台的匿名聚合统计，不是单个用户信息。部分文章可能因发布时间、统计周期或样本量原因返回空画像，这是后台没有提供数据，不代表程序漏抓。
...
全量导出会在所有文章处理完成后写入 `summary.csv` 和 `summary.json`。运行前请关闭 Excel、编辑器或其他程序中已打开的这两个文件；如果文件被占用，程序会在开始写文章前直接报错，避免留下不完整的本次导出结果。
...
每篇 Markdown 文件的 front matter 中会包含文章元数据、发布详情字段和文章分析字段；`summary.csv` / `summary.json` 会保存同一组详情字段，适合后续做表格分析。`publish` 模式会自动保留后台发布详情中返回的非空字段，微信后台字段增加时无需修改字段白名单。
